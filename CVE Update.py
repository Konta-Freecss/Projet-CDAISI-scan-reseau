...
cve_cpt = 0
while ok:
    with open('cve.json', encoding='utf-8') as json_file:
    ...
    for o in range(0,len(rows),2):
        ...
        if actual_cve_number > number_cve:
            ok = False
            break
        cve_data = {'CVE_ID':'','CWE_ID':'','#':'','Vulnerability':'','Publish Date':'','Update Date':'','Score':'','Gained Access Level':'','Access':'','Complexity':'','Authentication':'','Conf':'','Integ':'','Avail':'','Description':'', 'Products Affected':[]}
        ...
        try:
            tab_product_affected = WebDriverWait(driver, 10).until(EC.presence_of_element_located((By.ID, "vulnprodstable"))).find_element(By.TAG_NAME,'tbody').find_elements(By.TAG_NAME,'tr')[1:]
            #tab_product_affected = driver.find_element(By.ID,'vulnprodstable').find_element(By.TAG_NAME,'tbody').find_elements(By.TAG_NAME,'tr')[1:]
            for tr in tab_product_affected:
                data_product = {'id': '', 'Product Type': '', 'Vendor': '', 'Product': '', 'Version': '', 'Update': '', 'Edition': '','Language': ''}
